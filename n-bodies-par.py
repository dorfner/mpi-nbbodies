...
def init_world(n):
    data = [ create_item(id=i,
        positionx=1e18*math.exp(-1.8)*(.5-random.uniform(0,1)),
        positiony=1e18*math.exp(-1.8)*(.5-random.uniform(0,1)),
        speedx=0, speedy=0,
        weight=(random.uniform(0,1)*solarmass*10+1e20))
        for i in range(n-1)]
#        for i in range(n-2)]
#    data.append( create_item(id=nbbodies-1,
#        positionx=2e17, positiony=8e16,
...
dataShare = np.empty((shareSize, 6), dtype='f8')
...
for t in range(NBSTEPS):
    ...
    for i in range(shareSize):
        for j in range(nbbodies):
            force[i] = force[i] + interaction(dataShare[i], data[j])

    for i in range(shareSize):
        dataShare[i] = update(dataShare[i], force[i])
